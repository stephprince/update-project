...
for metricIdx = 1:size(metrics)
  for axIdx = 1:2
    for turnIdx = 1:2
      for outIdx = 1:2
        %get data and trial types to look at
        metric2plot = metrics{metricIdx};
        trialtype1 = eval(trialtypes.([trialOutcome{outIdx} 'Trials']));
        trialtype2 = eval(trialtypes.([trialTurn{turnIdx} 'Trials']));
        metricHist = eval(hists2plot.([metric2plot 'Hists']).(['hist' trialPlottype{axIdx}]));
        ...
        posBinsAll = eval(hists2plot.([metric2plot 'Hists']).['pos' trialPlottype{axIdx} 'bins']);
