...
if ~isempty(trackdata.durAll)
    ...
    for fIdx = 1:length(fieldGroups)
        ...
        %plot data in normal distribution form
        params.histedgesize = (max(trialDurs)-min(trialDurs))/20;
        histedges = min(trialDurs):params.histedgesize:max(trialDurs);
        trialDursCorrect = trackdata.(fnames{fieldsToPlot(fieldIdx)});
        trialDursIncorrect = trackdata.(fnames{fieldsToPlot(fieldIdx+1)});
        trialDursFailed = trackdata.(fnames{fieldsToPlot(fieldIdx+2)});
        trialDursCorrectHist = histcounts(trialDursCorrect,histedges)/sum(histcounts(trialDursCorrect,histedges));
        trialDursIncorrectHist =  histcounts(trialDursIncorrect,histedges)/sum(histcounts(trialDursIncorrect,histedges));
        trialDursFailedHist =  histcounts(trialDursFailed,histedges)/sum(histcounts(trialDursFailed,histedges));
        figure; hold on;
        plot(histedges(1:end-1),trialDursCorrectHist,'g-');
        plot(histedges(1:end-1),trialDursIncorrectHist,'r-');
        plot(histedges(1:end-1),trialDursFailedHist,'k-');
        xlabel('Duration (s)'); set(gca,'tickdir','out');
        title(['S' num2str(animal) ' performance on ' track ' track - ' fnames{fieldsToPlot(fieldIdx)}]);
        filename = [dirs.behaviorfigdir fnames{fieldsToPlot(fieldIdx)} '_' track  '_S' num2str(animal) '_hist'];
        saveas(gcf,filename,'png'); saveas(gcf,filename,'fig');
