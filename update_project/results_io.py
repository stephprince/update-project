...
class ResultsIO:
    ...
    def __init__(self, creator_file, git_hash=git_hash, base_path=base_path, session_id=None, folder_name=None):
    ...
    def get_results_path(self, results_type='group', results_name=None):
    ...
    def get_data_path(self, results_type='group', results_name=None):
        data_path = self.get_results_path(results_type, results_name=results_name) / 'intermediate_data'
        Path(data_path).mkdir(parents=True, exist_ok=True)

        return data_path

    def get_figure_args(self, filename, results_type='group', results_name=None, format='pdf'):
        results_path = self.get_results_path(results_type, results_name)
        fname = results_path / f'{filename}_git{git_hash}.{format}'
    ...
    def get_data_filename(self, filename, results_type='group', results_name=None, format='npy'):
        data_path = self.get_data_path(results_type, results_name)
        fname = data_path / f'{filename}_git{git_hash}.{format}'
