function plotPercentCorrect(trackdata,animal,track,dirs,plotInfo)
%SP 190929

%% get fieldnames to plot
fnames = fieldnames(trackdata);
fieldsToPlot = find(~cellfun(@isempty,strfind(fnames,'perCorrect'))); %finds all the fieldname indices in the data structure that match perCorrect
...
for fieldIdx = 1:length(fieldsToPlot) 
    ...
    set(gca,'tickdir','out'); ylim([0 1.01]);
