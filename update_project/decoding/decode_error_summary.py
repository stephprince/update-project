...
if overwrite_data or not Path(intermediate_data_path / 'decoding_error_group_summary_xpos.csv').is_file():
    decode_df_list = []
    for name, session in unique_sessions:
        ...
        time_support_all = nap.IntervalSet(start=non_update_epochs['start_time'], end=non_update_epochs['stop_time'],
                                           time_units='s')
        time_support_train = nap.IntervalSet(start=train_data['start_time'], end=train_data['stop_time'], time_units='s')
        time_support_test = nap.IntervalSet(start=test_data['start_time'], end=test_data['stop_time'], time_units='s')
    ...
    all_decoding_data.to_csv(intermediate_data_path / 'decoding_error_group_summary.csv')
else:
    all_decoding_data = pd.read_csv(intermediate_data_path /'decoding_error_group_summary.csv')
...
# # plot histogram of all decoding errors
# axes['D'] = sns.histplot(data=all_decoding_data, x='decoding_error', element='step', color='k', alpha=0.25, stat='percent', ax=axes['D'])
# axes['D'].set_title('Group error distribution')
# axes['D'].set(xlabel='Error', ylabel='Proportion')
#
# axes['E'] = sns.histplot(data=session_rmse, x='session_rmse', element='step', stat='percent', color='k', ax=axes['E'])
# axes['E'].set_title('Group error distribution')
# axes['E'].set(xlabel='RMSE', ylabel='Proportion')
...
Path(figure_path).mkdir(parents=True, exist_ok=True)
filename = figure_path / f'decoding_error_summary_git{short_hash}.pdf'
