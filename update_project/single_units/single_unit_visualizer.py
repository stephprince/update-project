...
class SingleUnitVisualizer:
    ...
    def plot(self):
        for g_name, g_data in self.aggregator.group_tuning_curves.groupby(['region', 'feature_name']):
            self.plot_update_selectivity(g_data, g_name)
    ...
    def plot_update_selectivity(self, g_data, g_name):
        ...
        # plot heatmaps of cell spatial maps
        # note these are generated by the non-update trials so only have that data even when sort by switch/stay index
        plot_dict = dict(switch_vs_non_update_index=dict(row=0, cutoff=0.25, sort_by='switch_vs_non_update_index',
                                                         psths=['mean_switch', 'mean_non_update', 'diff_switch_non_update'],
                                                         cmaps=['cmap_r', 'cmap_r', 'div_cmap'], ),
                         switch_vs_stay_index=dict(row=1, cutoff=0.25, sort_by='switch_vs_stay_index',
                                                   psths=['mean_switch', 'mean_stay', 'diff_switch_stay'],
                                                   cmaps=['cmap_r', 'cmap_r', 'div_cmap']))
        for metric, value in plot_dict.items():
            # plot psths for all cells sorted by selectivity
            data = g_data.sort_values(value['sort_by']).reset_index(drop=True)
            for col, p in enumerate(value['psths']):
                psth_mat = np.stack(data[f'psth_{p}'].to_numpy())
                ...
                im = axes[value['row']][col].imshow(psth_scaled, cmap=self.colors[value['cmaps'][col]],
                                                    origin='lower', aspect='auto', vmin=min_scale, vmax=max_scale,
                                                    extent=[x_limits[0], x_limits[1], y_limits[0], y_limits[1]])
                axes[value['row']][col].set(title=p, xlabel='time around update', ylabel='unit')
                axes[value['row']][col].axvline(0, linestyle='dashed', color='k')
                plt.colorbar(im, ax=axes[value['row']][col], pad=0.04, location='right', fraction=0.046,
                             label='Firing rate')
            ...
            y_limits = [0, np.shape(tuning_curve_scaled)[0]]
            x_limits = [np.round(np.min(tuning_curve_bins), 2), np.round(np.max(tuning_curve_bins), 2)]
            im_tuning = axes[value['row']][col].imshow(tuning_curve_scaled, cmap=self.colors['cmap_r'],
                                                     origin='lower', vmin=0.1, vmax=0.9, aspect='auto',
                                                     extent=[x_limits[0], x_limits[1], y_limits[0], y_limits[1]])
            plt.colorbar(im_tuning, ax=axes[value['row']][col], pad=0.04, location='right', fraction=0.046,
                         label='Normalized firing rate')

            # plot annotation lines
            locations = self.virtual_track.cue_end_locations.get(g_data['feature_name'].values[0], dict())  #TODO - generalize
            for key, val in locations.items():
                axes[value['row']][col].axvline(val, linestyle='dashed', color='k', alpha=0.5)
            axes[value['row']][col].set(xlim=x_limits, ylim=y_limits, ylabel='units', xlabel='position',
                                       title=f'{metric} - tuning curves')
            for s in self.virtual_track.edge_spacing:
                axes[value['row']][col].axvspan(*s, color='#DDDDDD', edgecolor=None)
            for col in [0, 1, 2]:
                cutoff1 = data.query(f'{value["sort_by"]} >= {value["cutoff"]}').index[0]
                cutoff2 = data.query(f'{value["sort_by"]} <= -{value["cutoff"]}').index[-1]
                axes[value['row']][col].axhline(cutoff1, linestyle='dashed', linewidth=2, color='w')
                axes[value['row']][col].axhline(cutoff2, linestyle='dashed', linewidth=2, color='w')
