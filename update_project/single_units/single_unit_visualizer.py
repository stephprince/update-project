...
class SingleUnitVisualizer:
    ...
    def plot(self):
        for g_name, g_data in self.aggregator.group_tuning_curves.groupby(['region', 'feature_name']):
            ...
            self.plot_update_selective_cell_types(g_data, g_name)
    ...
    def plot_update_selective_cell_types(self, g_data, g_name):
        cutoffs = [self.cutoff, -self.cutoff]
    ...
    def plot_update_selectivity(self, g_data, g_name, plot_dict=None):
        # plot heatmaps of cell spatial maps
        # note these are generated by the non-update trials so only have that data even when sort by switch/stay index
        ...
        for metric, value in plot_dict.items():
            ...
            for row, cond in enumerate(value['conditions']):
                ...
                data = data[~zero_max.any(axis=1)]  # remove any rows where cell has 0 max firing rate
                for col, p in enumerate(value['psths']):
                    psth_mat = np.stack(data[f'psth_{p}'].to_numpy())
                    if cmaps[col] == 'div_cmap':  # scale to -1, +1
                        ...
                        vmin, vmax = 0.25, 0.75
                        maxes = np.nanmax(psth_mat, axis=1)[:, None]
                        maxes[maxes == 0.0] = 1  # so units with no spikes get counted as 0 not nan
                        psth_scaled = psth_mat /maxes
                    y_limits = [0, np.shape(psth_mat)[0]]
                    x_limits = [data['psth_times'].values[0].min(), data['psth_times'].values[0].max()]
                    im = axes[row * 2][col].imshow(psth_scaled, cmap=self.colors[cmaps[col]],
                                                        origin='lower', aspect='auto', vmin=vmin, vmax=vmax,
                                                        extent=[x_limits[0], x_limits[1], y_limits[0], y_limits[1]],
                                                       )
                    axes[row * 2][col].set(xlabel='time around update', ylabel='unit')
                    axes[row * 2][col].axvline(0, linestyle='dashed', color='k')
                    plt.colorbar(im, ax=axes[row * 2][col], pad=0.04, location='right', fraction=0.046,
                                 label='Firing rate')
                ...
                for key, val in conditions.items():
                    ...
                    y_limits = [0, np.shape(tuning_curve_scaled)[0]]
                    x_limits = [np.round(np.min(tuning_curve_bins), 2), np.round(np.max(tuning_curve_bins), 2)]
                    im_tuning = axes[row * 2][val['col']].imshow(tuning_curve_scaled, cmap=self.colors['cmap_r'],
                                                             origin='lower', vmin=0.25, vmax=0.75, aspect='auto',
                                                             extent=[x_limits[0], x_limits[1], y_limits[0], y_limits[1]])
                    axes[row * 2][val['col']].set(title=f'{key} {new_line} sorted by {val["sort_by"]}', xlabel='position')
                    ...
                    for s_name, s_data in t_data.groupby(val['group_by'], sort=False):
                        ...
                        axes[row * 2][val['col']].axvspan(0, 20, ymin=counter/len(t_data),
                    ...
                    for k, v in locations.items():
                        axes[row * 2][val['col']].axvline(v, linestyle='dashed', color='k', alpha=0.5)
                ...
                plt.colorbar(im_tuning, ax=axes[row * 2][val['col']], pad=0.04, location='right', fraction=0.046,
                             label='Normalized firing rate')

            # save figure
            fig.suptitle(f'Goal selectivity - {g_name} - {metric}')
            self.results_io.save_fig(fig=fig, axes=axes, filename=f'update_selectivity_{metric}_{cutoffs[0]}',
