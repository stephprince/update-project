...
def show_start_aligned_psth(start_aligned, note_times, group_inds,
                            end=0,
    ...
    if len(annotation_times['t_delay']) < len(start_aligned):
        for key, times in annotation_times.items():  # replicate the matrix so same number as y_to_fill
            annotation_times[key] = np.array([times] * len(start_aligned)).flatten()
    ...
    for ax in axes:
        ...
        ax.fill_betweenx(y_to_fill_no_gaps, annotation_times['t_choice_made'], np.zeros(len(y_to_fill))+end,
        ...
        show_psth_raster(start_aligned, ax=ax, start=0, end=end, group_inds=group_inds, labels=labels)
...
def show_event_annotations(ax, annotation_times, y_length):
    ...
    ax.annotate('delay on (grey)', (annotation_times['t_delay'][0], 0), xycoords='data',
                ...
                arrowprops=dict(arrowstyle='->'))
    if np.size(annotation_times['t_update'][update_bool]):
...
def get_annotation_times(trials):
    # get trials to use for different alignment events
    alignment_trial_inds = {'start_time': range(len(trials)),
                            't_delay': np.argwhere(~np.isnan(trials['t_delay'][:]).values).squeeze(),
                            't_update': np.argwhere((trials['update_type'][:] != 1).values).squeeze(),
                            't_delay2': np.argwhere(~np.isnan(trials['t_delay2'][:]).values).squeeze(),
                            't_choice_made': range(len(trials))}

    # get delay times and use to sort trials
    annotation_times = dict()
    for key in alignment_trial_inds.keys():
        annotation_times[key] = trials[key][:] - trials['start_time'][:]
