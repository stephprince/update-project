classdef ViolinSP < handle
    ...
    properties
        ScatterPlot1 % scatter plot of the data points
        ScatterPlot2 % scatter plot of the data points
        ViolinPlot1  % fill plot of the kernel density estimate
        ViolinPlot2  % fill plot of the kernel density estimate
    ...
    methods
        function obj = ViolinSP(data, pos, varargin)
            ...
            if min(data) == max(data)
                density = 1;
            elseif abs(min(data)-max(data)) < 10^-13  %&& sum(data) > 10^-14 %check if data is identical and account for rounding errors
                densitytemp = 1;      
            end
            ...
            if exist('densitytemp','var') %data is identical with weird things i have to account for smp
                jitterstrength = densitytemp*width;
            elseif length(density) > 1
                jitterstrength = interp1(value, density*width, data);
            else % all data is identical:
                jitterstrength = density*width;
            end
            
            %get jitter splitting into the two populations
            jitter = 2*(rand(size(data))-0.5);
            negjitter = jitter<0; posjitter = jitter>0;
            if length(jitterstrength) == length(posjitter)
                jitterstrengthpos = jitterstrength(posjitter);
                jitterstrengthneg = jitterstrength(negjitter);
            else
                jitterstrengthpos = jitterstrength;
                jitterstrengthneg = jitterstrength;
            end
%             obj.ScatterPlot = ...
%                 scatter(pos + jitter.*jitterstrength, data, 'filled');

            obj.ScatterPlot1 = ...
                scatter(pos + jitter(posjitter).*jitterstrengthpos, data(posjitter), 'filled');
            obj.ScatterPlot2 = ...
                scatter(pos + jitter(negjitter).*jitterstrengthneg, data(negjitter), 'filled');
...
%                      [value value(end:-1:1)], [1 1 1]);
            obj.ViolinPlot1 =  ... % plot color will be overwritten later
                fill([pos+density*width pos-density(end:-1:1)*0], ...
                     [value value(end:-1:1)], [1 1 1]);
            obj.ViolinPlot2 =  ... % plot color will be overwritten later
            ...
            meanValue = mean(data);
            if exist('densitytemp','var') %data is identical with weird things i have to account for smp
                meanDensityWidth = densitytemp*width;
            elseif length(density) > 1
            ...
            if not(isempty(args.ViolinColor))
                ...
                obj.ViolinColor = obj.ScatterPlot1.CData;
                obj.ViolinColor = obj.ScatterPlot2.CData;
            end
        ...
        function set.EdgeColor(obj, color)
            if ~isempty(obj.ViolinPlot1)
                obj.ViolinPlot1.EdgeColor = color;
        ...
        function color = get.EdgeColor(obj)
            if ~isempty(obj.ViolinPlot1)
                color = obj.ViolinPlot1.EdgeColor;
                color = obj.ViolinPlot2.EdgeColor;
            end
        end
        ...
        function set.ViolinColor(obj, color)
            obj.ViolinPlot1.FaceColor = color;
            obj.ViolinPlot2.FaceColor = color;
            obj.ScatterPlot1.MarkerFaceColor = color;
            obj.ScatterPlot2.MarkerFaceColor = color;
            obj.MeanPlot.Color = color;
        end

        function color = get.ViolinColor(obj)
            color = obj.ViolinPlot1.FaceColor;
            color = obj.ViolinPlot2.FaceColor;
        end

        function set.ViolinAlpha(obj, alpha)
            obj.ScatterPlot1.MarkerFaceAlpha = alpha;
            obj.ScatterPlot2.MarkerFaceAlpha = alpha;
            obj.ViolinPlot1.FaceAlpha = alpha;
            obj.ViolinPlot2.FaceAlpha = alpha;
        end

        function alpha = get.ViolinAlpha(obj)
            alpha = obj.ViolinPlot1.FaceAlpha;
            alpha = obj.ViolinPlot2.FaceAlpha;
        end

        function set.ShowData(obj, yesno)
            if yesno
                obj.ScatterPlot1.Visible = 'on';
                obj.ScatterPlot2.Visible = 'on';
            else
                obj.ScatterPlot1.Visible = 'off';
                obj.ScatterPlot2.Visible = 'off';
            end
        end

        function yesno = get.ShowData(obj)
            if ~isempty(obj.ScatterPlot1)
                yesno = strcmp(obj.ScatterPlot1.Visible, 'on');
                yesno = strcmp(obj.ScatterPlot2.Visible, 'on');
            end
        end
